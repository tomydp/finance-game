<div class="space-y-10 text-slate-900">
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-6">
    ...
    @foreach ([
      ...
      <div class="rounded-3xl border border-slate-300 bg-white p-5 shadow-lg shadow-slate-200/60">
        <div class="text-xs font-semibold uppercase tracking-wide text-slate-600">{{ $card[0] }}</div>
        <div class="mt-3 text-3xl font-semibold text-slate-900">{{ $card[1] }}</div>
      </div>
    @endforeach
  </div>

  {{-- Top curso --}}
  <div class="rounded-3xl border border-slate-300 bg-white p-6 shadow-lg shadow-slate-200/60">
    <div class="text-sm font-medium text-slate-600">Top curso (7d)</div>
    <div class="mt-2 text-xl font-semibold text-slate-900">{{ $kpis['top_curso_7d'] }}</div>
  </div>
  
  {{-- Charts --}}
  <div class="space-y-6">
    <div class="rounded-3xl border border-slate-300 bg-white p-6 shadow-xl shadow-slate-200/60">
      <h3 class="mb-4 text-lg font-semibold text-slate-900">Altas por día (30d)</h3>
      <div class="h-64">
    ...
    <div class="rounded-3xl border border-slate-300 bg-white p-6 shadow-xl shadow-slate-200/60">
      <h3 class="mb-4 text-lg font-semibold text-slate-900">Usuarios activos por día (30d)</h3>
      <div class="h-64">
    ...
    <div class="rounded-3xl border border-slate-300 bg-white p-6 shadow-xl shadow-slate-200/60">
      <h3 class="mb-4 text-lg font-semibold text-slate-900">Ejercicios por día (30d) + Precisión</h3>
      <div class="h-64">
    ...
    <div class="rounded-3xl border border-slate-300 bg-white p-6 shadow-xl shadow-slate-200/60">
      <h3 class="mb-4 text-lg font-semibold text-slate-900">Top cursos por actividad (7d)</h3>
      <div class="h-72">
    ...
    <div class="rounded-3xl border border-slate-300 bg-white p-6 shadow-xl shadow-slate-200/60">
      <h3 class="mb-4 text-lg font-semibold text-slate-900">Tipos de ejercicio (7d)</h3>
      <div class="h-64">
...
(function(){
  ...
  // Siempre usar colores para modo claro
  const palette = {
    text: '#0f172a',
  ...
  new Chart(document.getElementById('types7'), {
    ...
    data: {
      ...
      datasets: [{
        ...
        borderColor: types.map((_, idx) => withAlpha(typeColors[idx % typeColors.length], 1)),
        borderWidth: 1,
      }]
    },
    options: { responsive: true, maintainAspectRatio: false }
